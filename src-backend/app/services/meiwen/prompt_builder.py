...
3. **直击痛点**：不要只罗列吉凶，要结合用户具体问的事情（如感情、财运、决策），给出具体的行动建议。"""
    ...
    @staticmethod
    def build_user_prompt(question: str, data: dict) -> str:
        """
...
"""
        ...
        for line in reversed(base["lines"]):
            ...
            line_str = (
                f"- {pos}: {god} {rel} {line['branch']}{line['element']} {move} {role}{hidden_str}"
            )
            prompt += line_str + "\n"
        ...
        if bian:
            ...
            for line in reversed(bian["lines"]):
                prompt += (
                    f"- 第{line['position']}爻: {line['relation']} {line['branch']}{line['element']}\n"
                )
        else:
            prompt += "\n(此卦为静卦，无变爻)\n"
